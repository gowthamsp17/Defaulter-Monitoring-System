...
export default function DefaulterList() {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get('http://192.168.239.7:5001/defaulterFetch');
  ...
  const handleFilter = async () => {
    try {
      ...
      const response = await axios.get(`http://192.168.239.7:5001/defaulterFetch?fromDate=${fromDate}&toDate=${toDate}&department=${departmentDD}`);
      setDefaulterData(response.data);
